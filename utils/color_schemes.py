"""
Color schemes for the geographic dashboard.
...
# =============================================================================
# NEW: Multi-Layer Helper Functions
# =============================================================================
...
def calculate_dot_radius(participant_count: int, base_radius: int = 60) -> int:
    """
    ...
    Args:
        ...
        base_radius: Base radius in meters for minimum dot size

    Returns:
        Radius in meters for pydeck ScatterplotLayer
    """
    if participant_count == 0 or participant_count is None:
        return int(base_radius * 0.5)  # Small dot for no training

    # Sqrt scaling: proportional but not overwhelming
    # 1 participant = 0.85x base, 10 = 1.6x base, 50 = 3x base
    scale_factor = 0.5 + (participant_count ** 0.5) * 0.35
    return int(base_radius * min(scale_factor, 3.0))  # Cap at 3x
